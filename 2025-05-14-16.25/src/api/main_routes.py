...
def preview_file(filename):
    ...
    try:
        ...
        if order_id:
            ...
            preview_path = os.path.join(order_folder, filename)
            
            # 只有当文件存在于该订单目录时才允许访问
            if os.path.exists(preview_path):
                current_app.logger.info(f"文件存在于订单 {order_id} 目录中")
                file_dir = order_folder
                file_name = filename
                return send_from_directory(file_dir, file_name, mimetype='image/png')
            else:
                # 尝试在根目录中查找旧文件（仅当提供了订单ID且归属于该订单）
                root_path = os.path.join(converted_folder, filename)
                if os.path.exists(root_path):
                    # 在根目录发现文件，检查文件是否属于该订单
                    ...
                    if converted_file and str(converted_file.order_id) == str(order_id):
                        current_app.logger.info(f"文件在根目录中找到，且属于订单 {order_id}")
                        return send_from_directory(converted_folder, filename, mimetype='image/png')
                    else:
        ...
        with zipfile.ZipFile(zip_path, 'w') as zipf:
            ...
            for file_obj in converted_files_objs:
                ...
                if os.path.exists(file_path):
                    ...
                    order_folder = os.path.join(converted_folder, f"order_{order.id}")
                    alt_file_path = os.path.join(order_folder, filename)
                    
                    if os.path.exists(alt_file_path):
                        # 文件在订单子目录中找到
                        zipf.write(alt_file_path, filename)
                        current_app.logger.info(f"添加文件到ZIP (从订单目录): {alt_file_path} -> {filename}")
                    else:
                        # 尝试在根目录查找
                        root_file_path = os.path.join(converted_folder, filename)
                        if os.path.exists(root_file_path):
                            zipf.write(root_file_path, filename)
                            current_app.logger.info(f"添加文件到ZIP (从根目录): {root_file_path} -> {filename}")
                        else:
...
def download_file_by_name(filename):
    ...
    try:
        ...
        if order_id:
            ...
            if os.path.exists(file_path):
                current_app.logger.info(f"文件存在于订单 {order_id} 目录中")
                return send_from_directory(order_folder, filename, as_attachment=True, download_name=filename)
            else:
                # 尝试在根目录中查找旧文件
                root_path = os.path.join(converted_folder, filename)
                if os.path.exists(root_path):
                    ...
                    if converted_file and str(converted_file.order_id) == str(order_id):
                        current_app.logger.info(f"文件在根目录中找到，且属于订单 {order_id}")
                        return send_from_directory(converted_folder, filename, as_attachment=True, download_name=filename)
                    else:
                        current_app.logger.warning(f"文件 {filename} 不属于订单 {order_id}")
